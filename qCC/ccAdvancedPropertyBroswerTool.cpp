...
#include "ccAdvancedPropertyBrowserVariantManager.h"
#include "ccAdvancedPropertyVariantFactory.h"
#include <QMetaEnum>

//#include <ccAdvancedPropertyExtendedVariantManager.h>

PropertyBrowser::PropertyBrowser(QWidget *parent, ccDBRoot *root)
    : QtTreePropertyBrowser(parent), m_dbroot(root) {
    setRootIsDecorated(true);
    setAlternatingRowColors(true);

    connect(m_dbroot, SIGNAL(selectionChanged()), this, SLOT(selectionChanged()));
...
void PropertyBrowser::selectionChanged()
{
        ccHObject::Container objects;
        m_dbroot->getSelectedEntities(objects);

        QList<QObject *> good;
        for (ccHObject * obj: objects)
        {
            QObject * ob = dynamic_cast<QObject *> (obj);
            if (ob)
                good.push_back(ob);
        }
        ...
        if (!good.empty())
        {
            QList<QObject *> mirrors;
                ...
                {
                    mirrors.append(good.at(i));
                }

                this->setObject(good.at(0), mirrors);
        }
